...
\texttt{TinkerGraph} is the default graph implementation that the standard BluePrints implementations package called Tinkerpop, uses in order to implement a grpah structure. This is a full featured property graaph. -> large memory overhead in storing useless features. Uses HashMaps and Sets. -> Stores Edges as seperate objects ( This is the main drawback)

While doing experiments with the imdb databse (describe), with cast\_info table having 30M rows etc...While trying to use the condensed representation mentioned in section x, we found that we would run out of memory with tinkergraph. With our custom graph implementation, however, we were able to fit the entire condensed graph in memory (fingers crossed I find the bug). A graph with over a billion edges, condensed. Very powerful stuff.

We researched In-Memory Graph Representations. People use CSR {cite ppl who use CSR}.

There is LLAMA, but it's meh.

Thing is, CSR is immutable... we NEED to create the graph while constructing it, and since thered doesn't seem to be any substantial overhead in our approach , at least at a high level, we used combination of Adjacency Map and Adjacency lists, with a CSR flavor. In graph analysis and machine learning tasks, the most important operation, that is crucial to be supported in as efficient a manner as possible, is iterating over the INcoming and OUTgoing edges/ neighbors. Since the only operation that we can't perform efficiently in theory is deletion of an edge, and it doesn't seem that deletion is something that is too important to be supported efficiently, there doesn't seem to be an issue with this implementation.

\ref{fig:ourGraphImpl}
