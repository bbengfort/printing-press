...
We conducted two primary experiments to test the behavior of a federated consistency system against homogenous Raft and homogenous Eventual systems. The first was to test the behavior of increasing conflict, $P_c$ and the second was to explore the effect of the network environment in terms of the mean latency, $\lambda{\mu}$ of the wide area. The input to the system was a fixed topology of 20 nodes in 5 wide areas, along with an access trace that contained approximately 29,000 accesses (depending on the experiment), with an average ratio of 54\% of the accesses being reads.

Each simulation measured the performance of accesses by measuring \textit{forked writes} and \textit{stale reads}. Forked writes were the number of writes in the simulation that had more than one child whereas stale reads where the number of read accesses that read a version that was behind the globally latest version. A further metric of \textit{inconsistent writes} measured the number of writes whose parent was forked allowed into the log of a replica, as balanced by the number of \textit{dropped writes} -- writes that the sequential ordering Raft nodes would reject as being invalid.

Furthermore the cost of each replication protocol was measured in terms of the number of \textit{sent messages} as well as latencies of various RPC calls. Some latencies were dependent if the protocol specified \textit{cached} or \textit{remote} accesses: the \textit{read latency} and \textit{write latency} measured the amount of time an access took until a response was returned to the client. In the case of Eventual nodes, this latency was zero, since an eventual node always responds with cached information; however Raft required remote accesses to the leader in order to respond. Other latencies were global latencies, e.g. the \textit{visibility latency} specified the average amount of time it took a write to become fully visible, that is replicated to all nodes in the network. Any writes that did not become fully visible (stomped on through the eventually consistent policy of latest writer wins) were ignored, but this metric is closely in line with the \textit{percent visible} metric -- the average number of replicas a write is propagated to. Finally the \textit{commit latency} specified how much time was required for a write to be committed, e.g. confirmed as accepted by a quorum of nodes.
...
As shown in Figure \ref{fig:inconsistent_writes}, as the conflict in the system increases, eventual methodologies suffer from more and more inconsistent writes. Eventual nodes do manage forks or ordering of accesses, but simply write to the latest version. This has benefits in that eventual nodes can respond quickly, and in fact as shown in \ref{fig:stale_reads}, eventual systems perform well with minimizing the number of stale reads even as conflict increases. However, as shown in \ref{fig:forked_writes}, the inconsistencies that are in the eventual nodes match the number of forked writes in the system. However, both Raft and Federated systems eliminate inconsistencies (Federated allows a few in higher conflict scenarios) by simply dropping writes. This provides more orderly consistent behavior overall.
...
\begin{figure*}
    ...
    \minipage{0.5\textwidth}
      ...
      \caption{The write latency for eventual and sequential homogenous systems compared to the federated system.}\label{fig:write_latency}
    \endminipage\hfill
    \minipage{0.5\textwidth}
      \includegraphics[width=\linewidth]{figures/latency/visibility_latency}
      \caption{The visibility latency of the mdoel..}\label{fig:visibility_latency}
    \endminipage\hfill
\end{figure*}
...
\begin{figure*}
    ...
    \minipage{0.32\textwidth}
      ...
      \caption{The number of forked writes that are allowed into the log.}\label{fig:latency_inconsistent_writes}
    \endminipage\hfill
    \minipage{0.32\textwidth}
      \includegraphics[width=\linewidth]{figures/latency/forked_writes}
      \caption{The number of writes that are forked for eventual and sequential homogenous systems compared to the federated system.}\label{fig:latency_forked_writes}
    \endminipage\hfill
    \minipage{0.32\textwidth}%
      \includegraphics[width=\linewidth]{figures/latency/stale_reads}
      \caption{The number of stale reads for eventual and sequential homogenous systems compared to the federated system.}\label{fig:latency_stale_reads}
    \endminipage
\end{figure*}

In the second experiment we ran 216 experiments to test the affect of variable network latency on consistency protocols as well as on how the selection of the tick parameter model affects consistency for each system. We specified 12 wide area mean latencies in three categories: low ($\lambda{\mu} \in [30,1000]ms$ with a low $\lambda_{\sigma}$), medium ($\lambda{\mu} \in [1000,2000]ms$ with a low $\lambda_{\sigma}$), and high ($\lambda{\mu} \in [2000,3000]ms$ with a high $\lambda_{\sigma}$). In each of these environments we ran three replication models: \textit{homogenous eventual}, \textit{homogenous Raft}, and \textit{federated eventual and Raft}.

Each of these 108 simulations was then parameterized with two different $T$ parameters, computed by the wide area $\lambda_{\mu}$ and $\lambda_{\sigma}$: $T_{conservative}$ and $T_{optimistic}$. However the access mean, $A_{\mu}$ was fixed at approximately one access per replica every 3000ms. In affect, this meant that for approximately half the simulations (with the higher latencies), it was impossible for a write to become visible on another replica before a fork. Even though we fixed the conflict probability as $P_c=0.5$ there was still enough conflict in the simulation that forced each replica protocol to handle forks.

As shown in \ref{fig:write_latency}, both Raft and Federated consistency models are hugely dependent on the mean wide area latency for remote accesses. This is because the single Raft leader in both models is necessarily in a remote area for the majority of the nodes, requiring communication over the more unstable links. Federated, however, can get away with fewer writes to the leader in this environment, and therefore has a lower aggregate write latency, even at the extreme wide area latencies. The visibility latency, the average time it takes a write to become fully visible is also worse for Raft and Eventual than it is for Federated as shown in \ref{fig:visibility_latency}. Eventual must perform anti-entropy around the wide area and requires at least 4 anti-entropy sessions in order to become fully visible (as defined by the $T$ parameter). Raft can get away with full replication via broadcast over append entries RPCs, but due to out of order \texttt{AppendEntries} messages, can start to fall apart at the lower latencies.

In terms of consistency metrics, the number of inconsistent writes is more than halved from eventual in Federated due to the presence of the Raft core \ref{fig:latency_inconsistent_writes}. This is because Federated can take advantage of the topology where Raft and Eventual cannot - broadcasting across the wide area to minimize the number of pairwise communications in anti-entropy but still allowing available responses with the Eventual core. Both forks in Figure \ref{fig:latency_forked_writes} and stale reads in Figure \ref{fig:latency_stale_reads} are minimized.
