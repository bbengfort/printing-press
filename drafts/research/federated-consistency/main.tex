...
The rise of on-demand computing resources and the Cloud have made distributed systems the
...
Because we take a client-centric approach we will describe ,
...
Each replica sends messages to other nodes in order to propagate new writes.
...
originating node in a \texttt{GossipResponse} message.
...
parameter, $T$ (discussed below), and $N$ is the number of nodes in the system.
...
In this type of topology there are two types of failure. 
A \textit{node failure} occurs when a single node is shut off or stops responding to
messages.
A \textit{network partition} occurs when it is not possible for messages to be sent or
received from a single geographic region.
In both cases, 
accesses may continue at a partitioned
node or subnet, though they are not immediately replicated in the system, and
partitioned nodes may fall behind the global state. Correctness requires the system to re-integrate the nodes
when they re-join the network. 
...
Topologies specify each device as an independent replica server by uniquely identifying it
with device-specific configurations.
By far the most important configuration is the \textit{replica consistency} (or replica
protocol) which determines the replica's behavior in the environment:
\begin{itemize}
    \item \textbf{Eventual}: Eventual nodes perform replication via anti-entropy by random
    ...
    \item \textbf{Raft}: Raft nodes implement the Raft consensus protocol, electing a
...
% , and \textit{federated eventual with 5 stentor nodes}
...
as randomized anti-entropy partner selections allow writes to propogate through multiple
paths. 
...
The multiple-paths ability also allows the federated system to propogate writes quickly,
...
Figures~\ref{fig:visible_writes} and \ref{fig:visibility_latency} show that that write
