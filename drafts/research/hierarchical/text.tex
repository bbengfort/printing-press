...
% TODO
Though most systems are optimized for only one type of access pattern, real-world
applications vary in their needs for availability and durability, and manifest
markedly different patterns of accesses, including:

\begin{itemize}
    \item single ownership: objects are accessed in one region and do not migrate.
    \item revolving access: objects are accessed from multiple regions, but only
...
entire epoch, reconfiguration can be used to determine optimal placement rules
for access patterns and durability requirements.

To maximize throughput with no need for strong durability, a \sub can be placed in a
single region using Raft.
