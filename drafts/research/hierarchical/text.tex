...
links.
% We imply that the above algorithms do not scale, we may need to explictly state and
...
a 2 phase ballotting process: \texttt{PREPARE} and \texttt{ACCEPT}.
...
selecting a replica that has sole responsiblity of slot nomination.
...
allowing an auxillary master quorum to execute safe reconfiguration directives in the
...
\begin{figure}[t]
    ...
    \caption{\Hc is an extension of Vertical Paxos such that the \roo moves the system
...
transactions.
% This last sentence is kind of wierd
...
% the order of the section or use section references ... then tries to seque.
...
\pjk{description of protocol and implementation should be descrete}
