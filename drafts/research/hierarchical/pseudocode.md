...
**epoch**: a monotonically increasing logical time element that defines subquorums and their assigned tags. We've further optimized the root quorum to also consider the epoch as the term of the root leader (<span style="color:red">this implies we need to elect a new leader for every command!</span>). The epoch is equivalent to a _configuration_ in VP
