...
3. If there is a (q,t) conflict - e.g. the root receives two batches of delegated votes from the same subquorum, it resolves the number of votes by selecting the delegation with the highest subquorum term in case a failure in the subquorum caused a new leader election. (**PJK: example: 3 SQ of three each. Root election in progress w/ two candidates. SQ 1 votes for candidate A, SQ 2 votes for B. SQ 3 votes for A and immediately fails w/o telling its followers, and then new leader for SQ 3 votes for B. Should root votes be only undelegated?**)
